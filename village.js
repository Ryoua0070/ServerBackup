...
// 첫 입장 안내
if (!player.getDynamicProperty("VillageGuide")) {
...
async showVisitList(player) {
    ...
    form.body("방문할 마을을 선택하세요.");

    for (const village of villages) {

        form.button(
`${village.name}
§7Lv.${village.level} | ${village.members.length}명`
        );
...
world.beforeEvents.playerPlaceBlock.subscribe(event => {
    ...
    if (
        Math.abs(block.location.x - village.center.x) <= 1 &&
        Math.abs(block.location.z - village.center.z) <= 1 &&
        block.location.y === village.center.y
    ) {
